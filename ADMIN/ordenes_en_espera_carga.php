...
$id_usuario = $_SESSION["user_id"];
    ...
    if($action == 'ajax'){
        ...
        $query_a_buscar="SELECT count(*) AS numrows FROM ordenes_de_trabajo WHERE estado='PENDIENTE' ";
        if ($busqueda!="") {
...
<?php
    $query_a_ejecutar="SELECT * FROM ordenes_de_trabajo WHERE estado='PENDIENTE' ";
     if ($busqueda!="") {
...
<div class="card">
    <div class="card-tittle text-center fondo_verdecito sombra pt-1 pb-1" style="border-radius: 5px 5px 0px 0px;">
      <table width="100%">
        <tr>
          <td>
            <a href="#" onclick="updates_ordenes('APROBADA','<?php echo $id_orden; ?>');" class="btn btn-sm btn-success sombra"><i class="bi bi-hand-thumbs-up"></i></a>
          </td>
          <td>
            <a href="#" onclick="updates_ordenes('DECLINADA','<?php echo $id_orden; ?>');"  class="btn btn-sm btn-danger sombra"><i class="bi bi-hand-thumbs-down"></i></a>
          </td>
          <td width="90%">
